...
export default function Navigation({ overlay = false }: { overlay?: boolean }) {
  ...
  const navItems = [
    ...
    { href: '/services', label: ts('navigation.services') },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`${isTransparent ? 'text-white' : 'text-gray-600'} hover:text-apex-blue px-3 py-2 text-base font-medium transition-colors`}
                >
                  {item.label}
                </Link>
              ))}
              ...
              <div className="relative" ref={calculatorRef}>
                <button
                  ...
                  aria-haspopup="true"
                  className={`${isTransparent ? 'text-white' : 'text-gray-600'} hover:text-apex-blue px-3 py-2 text-base font-medium transition-colors flex items-center`}
                >
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-200" role="menu">
                    {calculatorItems.map((item) => (
                      <Link
                        ...
                        href={item.href}
                        role="menuitem"
                        className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-apex-blue transition-colors"
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-200" role="menu">
                    {languages.map((lang) => (
                      ...
                      >
                        ...
                        {language === lang.code && (
                          <svg className="ml-auto h-4 w-4 text-apex-blue" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
              ...
              <a
                ...
                className="flex items-center text-white bg-apex-red hover:bg-red-700 px-4 py-2 rounded-lg font-medium transition-colors"
              ...
              <a
                ...
                className={`flex items-center ${isTransparent ? 'text-white/90' : 'text-gray-600'} hover:text-apex-blue px-3 py-2 text-sm font-medium transition-colors`}
          ...
          <div className="md:hidden flex items-center space-x-2">
            ...
            <a
              href="tel:+18889907112"
              className="flex items-center justify-center text-white bg-apex-red hover:bg-red-700 p-2.5 rounded-lg transition-colors"
              aria-label="Call us"
            >
            ...
            <button
              ...
              className={`${isTransparent ? 'text-white' : 'text-gray-600'} hover:text-apex-blue inline-flex items-center justify-center p-3 rounded-md transition-colors`}
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-white/90 backdrop-blur-xl">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="text-gray-600 hover:text-apex-blue hover:bg-gray-100 block px-3 py-2 rounded-md text-lg font-medium transition-colors"
            ...
            <div className="px-3 py-2">
              ...
              {calculatorItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="text-gray-600 hover:text-apex-blue hover:bg-gray-100 block px-3 py-2 rounded-md text-base font-medium transition-colors ml-4"
                  onClick={() => setIsMenuOpen(false)}
                >
                  {item.label}
                </Link>
              ...
              {languages.map((lang) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-md text-base font-medium transition-colors ml-4 flex items-center ${
                    language === lang.code 
                      ? 'bg-blue-50 text-apex-blue' 
                      : 'text-gray-600 hover:text-apex-blue hover:bg-gray-100'
                  }`}
                >
                  <span className="mr-3">{lang.flag}</span>
                  <span>{lang.nativeName}</span>
            ...
            <div className="px-3 py-3 border-t border-gray-200 mt-2">
              ...
              <a href="tel:+17737821005" className="text-apex-blue font-semibold text-lg hover:text-apex-navy">(773) 782-1005</a>
            </div>
          </div>
        </div>
      )}
